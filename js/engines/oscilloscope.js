...
export default {
  ...
  label: "Oscilloscope",
  kind: "live",
  blurb: "A live CRT scope: a sine through a wavefolder, as a glowing phosphor trace. Switch to XY for Lissajous figures.",
  params: [
  ...
  mount(host, ctx) {
    ...
    const live = liveCanvas(host, {
      onFrame(c, w, h, t) {
        ...
        const trace = traceColor();
        const core = lighten(trace, 0.6);
        const midX = w / 2, midY = h / 2, ampX = w * 0.34, ampY = h * 0.34;

        // persistence: fade rather than clear — long trails at high persist
